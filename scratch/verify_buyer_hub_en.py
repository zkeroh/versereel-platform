...
def test_hub_en():
    ...
    hub_file = os.path.join(base_dir, 'access-dont-tell-dad-en.html')

    print("--- STEP 1: Verifying access-dont-tell-dad-en.html ---")
    if not os.path.isfile(hub_file):
        print("FAILED: access-dont-tell-dad-en.html missing!")
        return False
    ...
    for link in expected_links:
        if link not in content:
            print(f"FAILED: Link {link} not found in English hub page!")
            return False
        else:
            print(f"OK: Link {link} verified in English hub page.")
    ...
    print("\nSUCCESS: All English buyer hub links and target files are 100% verified!")
    return True
